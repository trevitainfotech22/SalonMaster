...
public class APB_LISTController implements Initializable {
...
//        Search bar and date bar
          ...
          datesearch.setOnMouseClicked((event) -> {
        ...
        try {
            ...
            String q = "SELECT ms_apb.id,ms_user.name, ms_user.phone_no, ms_apb.date, ms_apb.time FROM ms_apb JOIN ms_user ON ms_apb.user_id = ms_user.id ORDER BY ms_apb.id ASC;";
...
//            TABLE.setItems(list);
            ...
            filteredData = new FilteredList<>(list, b -> true);
            DateBar.valueProperty().addListener((observable, oldValue, newValue) -> {
                filteredData.setPredicate(APBLIST -> {
                    if (newValue == null) {
                        return true; // Show all items when no date filter is selected
                    }
                    LocalDate date = newValue;
                    LocalDate itemDate = APBLIST.getDate();

                    return itemDate != null && itemDate.equals(date);
                });
            });

            SortedList<APBLIST> sortedData = new SortedList<>(filteredData);
    ...
    public void bill(ActionEvent event) {
        ...
        BillingController billingFXMLDocumentController = loader.getController();
        billingFXMLDocumentController.setUpdate(true);
        billingFXMLDocumentController.setTextField(
                apblist.getName(),
                apblist.getPhone_no()
        );
        scene = new Scene(root);
